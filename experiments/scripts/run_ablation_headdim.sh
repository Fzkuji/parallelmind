...
# ============================================================================
# ParallelMind 消融实验 - Attention Head 维度测试（支持断点续训）
# ============================================================================
#
# 测试不同 attention head 维度对性能的影响
# 固定 hidden_size=512，调整 head_dim，num_heads 随之变化
#
# 功能：
...
#
# 用法：
#   ./run_ablation_headdim.sh           # 正常运行（跳过已完成的）
#   ./run_ablation_headdim.sh --force   # 强制重新运行所有实验
#
# 实验设计：
#   - head_dim: 32, 64, 128 (hidden_size=512, num_heads=16, 8, 4)
#   - rope_2d_ratio: 根据 head_dim 动态调整（见下方说明）
#   - 固定分支训练: 1, 2, 4, 8, 16
...
#   - head_dim=32  -> freq_count=16 -> 精度 6.25%
#   - head_dim=64  -> freq_count=32 -> 精度 3.125%
#   - head_dim=128 -> freq_count=64 -> 精度 1.5625%
#   - 所有 ratio (0, 0.25, 0.5, 0.75, 0.875, 1.0) 都有效
#
# 总实验数: 3 dims × 6 ratios × 9 configs = 162 个实验
#
# ============================================================================
...
NUM_GPUS=8
HIDDEN_SIZE=512          # 固定 hidden size
NUM_LAYERS=8
NUM_KV_HEADS=2

# Head 维度配置: "head_dim"
# num_heads = hidden_size / head_dim
HEADDIM_CONFIGS=(32 64 128)
...
# 日志目录（固定名称，便于续训）
LOG_DIR="experiments/logs/ablation_headdim"
mkdir -p "$LOG_DIR"
...
if [ ! -f "$CSV_FILE" ] || [ "$FORCE_RERUN" = true ]; then
    echo "head_dim,num_heads,rope_ratio,train_branch,eval_branch,loss,ppl" > "$CSV_FILE"
fi
...
TRAIN_CONFIGS=(
    # 只训练动态分支: 按平均分支数计算
    "1,3,8,2"      # avg=2,  batch=8,  accum=2, tokens≈131,072
    "1,7,4,2"      # avg=4,  batch=4,  accum=2, tokens≈131,072
    "1,15,2,2"     # avg=8,  batch=2,  accum=2, tokens≈131,072
    "1,31,1,2"     # avg=16, batch=1,  accum=2, tokens≈131,072
)
...
record_csv() {
    ...
    local LOSS=$6
    local PPL=$7
    echo "$HEAD_DIM,$NUM_HEADS,$ROPE_RATIO,$BRANCH_STR,$EVAL_BRANCH,$LOSS,$PPL" >> "$CSV_FILE"
}
...
run_training() {
    ...
    while [ $RETRY -lt $MAX_RETRIES ]; do
        ...
        TRAIN_CMD="torchrun --nproc_per_node $NUM_GPUS trainer/train_pretrain.py \
            --pe rope \
...
run_evaluation() {
    ...
    # 生成唯一 key
    local EXP_KEY="d${HEAD_DIM}-h${NUM_HEADS}-r${ROPE_STR}-b${BRANCH_STR}-eval${VAL_BRANCH}"
    ...
    while [ $RETRY -lt $MAX_RETRIES ]; do
        ...
        EVAL_CMD="python scripts/eval_loss.py \
        ...
        if [ $EVAL_EXIT -eq 0 ]; then
            ...
            if [ -n "$LOSS" ]; then
                record_csv "$HEAD_DIM" "$NUM_HEADS" "$ROPE_RATIO" "$BRANCH_STR" "$VAL_BRANCH" "$LOSS" "$PPL"
                log "[EVAL] Recorded: dim=$HEAD_DIM, heads=$NUM_HEADS, rope=$ROPE_RATIO, train=$BRANCH_STR, eval=$VAL_BRANCH, loss=$LOSS"
            fi
...
log ""
log "============================================================================"
log "ParallelMind 消融实验 - Attention Head 维度测试"
log "Started/Resumed at $(date)"
log "Log directory: $LOG_DIR"
log "Force rerun: $FORCE_RERUN"
log ""
log "实验配置:"
log "  head_dim: ${HEADDIM_CONFIGS[*]}"
log "  hidden_size: $HIDDEN_SIZE (固定)"
log "  rope_2d_ratio: ${ROPE_RATIOS[*]}"
log "  固定分支训练: 1, 2, 4, 8, 16"
log "  动态分支训练: 1-3, 1-7, 1-15, 1-31"
log "  评估分支: ${EVAL_BRANCHES[*]}"
TOTAL_EXPERIMENTS=$((${#HEADDIM_CONFIGS[@]} * ${#ROPE_RATIOS[@]} * ${#TRAIN_CONFIGS[@]}))
log "  总实验数: ${#HEADDIM_CONFIGS[@]} dims × ${#ROPE_RATIOS[@]} ratios × ${#TRAIN_CONFIGS[@]} configs = $TOTAL_EXPERIMENTS 个"
log "============================================================================"
...
# 遍历所有 head 维度配置
for HEAD_DIM in "${HEADDIM_CONFIGS[@]}"; do
    NUM_HEADS=$((HIDDEN_SIZE / HEAD_DIM))
    FREQ_COUNT=$((HEAD_DIM / 2))

    log ""
    log "############################################################################"
    log "# HEAD_DIM = $HEAD_DIM, NUM_HEADS = $NUM_HEADS"
    log "# freq_count = $FREQ_COUNT (ratio 精度: $(echo "scale=4; 100 / $FREQ_COUNT" | bc)%)"
    log "############################################################################"
    ...
    for i in "${!ROPE_RATIOS[@]}"; do
        ...
        record_loss ""
        record_loss "========================================"
        record_loss "HEAD_DIM=$HEAD_DIM, NUM_HEADS=$NUM_HEADS, ROPE_2D_RATIO=$ROPE_RATIO"
        record_loss "========================================"
        ...
        for CONFIG in "${TRAIN_CONFIGS[@]}"; do
            ...
            log ">>> DIM=$HEAD_DIM, HEADS=$NUM_HEADS, ROPE=$ROPE_RATIO, TRAIN_BRANCH=$BRANCH_STR"

            record_loss ""
            record_loss "--- EXPERIMENT $EXPERIMENT_COUNT / $TOTAL_EXPERIMENTS ---"
            record_loss "DIM=$HEAD_DIM, HEADS=$NUM_HEADS, ROPE=$ROPE_RATIO, TRAIN_BRANCH=$BRANCH_STR"
            record_loss "OUT_DIR: $OUT_DIR"

            # 训练
            if ! run_training "$HEAD_DIM" "$NUM_HEADS" "$ROPE_RATIO" "$MIN_BRANCH" "$MAX_BRANCH" "$BATCH_SIZE" "$ACCUM_STEPS" "$OUT_DIR"; then
            ...
            for VAL_BRANCH in "${EVAL_BRANCHES[@]}"; do
                ...
                run_evaluation "$MODEL_PATH" "$HEAD_DIM" "$NUM_HEADS" "$ROPE_RATIO" "$ROPE_STR" "$BRANCH_STR" "$VAL_BRANCH"
            done
...
echo "To force rerun all, use: ./run_ablation_headdim.sh --force"
echo "=========================================="
