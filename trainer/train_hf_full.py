...
def train_epoch(epoch, wandb):
    ...
    if (step + 1) % args.save_interval == 0 and (not ddp or dist.get_rank() == 0):
        ...
        ckpt_path = os.path.join(args.save_dir, "full", f"full_{args.model_tag}_step{step+1}.pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_tag", type=str, default="hf_full", help="保存模型文件时使用的后缀")
    ...
    Logger(f"可训练参数量: {trainable_params_count / 1e6:.2f}M ({trainable_params_count / total_params * 100:.2f}%)")
    ...
    if args.use_wandb and (not ddp or dist.get_rank() == 0):
        ...
        wandb.init(project=args.wandb_project, name=f"HF-FULL-{args.model_tag}")
    ...
    if (not ddp) or dist.get_rank() == 0:
        final_path = os.path.join(args.save_dir, "full", f"full_{args.model_tag}_final.pth")
        os.makedirs(os.path.dirname(final_path), exist_ok=True)
        model_to_save = model.module if isinstance(model, DistributedDataParallel) else model
        torch.save(model_to_save.state_dict(), final_path)
        Logger("=" * 80)
        Logger(f"训练完成！模型权重已保存至: {final_path}")
        Logger("=" * 80)
