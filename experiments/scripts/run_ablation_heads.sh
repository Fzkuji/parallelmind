...
# ============================================================================
# ParallelMind 消融实验 - Attention Head 数量测试（支持断点续训）
# ============================================================================
#
# 测试不同 attention head 数量对性能的影响（类似 scaling law）
# 固定 head_dim=64，调整 num_heads，hidden_size 随之变化
#
# 功能：
...
#
# 用法：
#   ./run_ablation_heads.sh           # 正常运行（跳过已完成的）
#   ./run_ablation_heads.sh --force   # 强制重新运行所有实验
#
# 实验设计：
#   - num_heads: 4, 8, 16, 24 (head_dim=64, hidden_size=256, 512, 1024, 1536)
#   - rope_2d_ratio: 0, 0.25, 0.5, 0.75, 0.875, 1.0
#   - 固定分支训练: 1, 2, 4, 8, 16
#   - 动态分支训练: 1-3, 1-7, 1-15, 1-31
#   - 评估分支: 1, 2, 4, 8, 16, 24, 32
#
# 总实验数: 4 heads × 6 ratios × 9 configs = 216 个实验
#
# ============================================================================
...
NUM_GPUS=8
HEAD_DIM=64              # 固定 head 维度
NUM_LAYERS=8
NUM_KV_HEADS=2

# Attention head 数量配置: "num_heads"
# hidden_size = num_heads * head_dim
HEAD_CONFIGS=(4 8 16 24)
...
# 日志目录（固定名称，便于续训）
LOG_DIR="experiments/logs/ablation_heads"
mkdir -p "$LOG_DIR"
...
record_csv() {
    ...
    local LOSS=$6
    local PPL=$7
    echo "$NUM_HEADS,$HIDDEN_SIZE,$ROPE_RATIO,$BRANCH_STR,$EVAL_BRANCH,$LOSS,$PPL" >> "$CSV_FILE"
}
...
run_evaluation() {
    ...
    # 生成唯一 key
    local EXP_KEY="h${NUM_HEADS}-r${ROPE_STR}-b${BRANCH_STR}-eval${VAL_BRANCH}"
    ...
    while [ $RETRY -lt $MAX_RETRIES ]; do
        ...
        if [ $EVAL_EXIT -eq 0 ]; then
            ...
            if [ -n "$LOSS" ]; then
                record_csv "$NUM_HEADS" "$HIDDEN_SIZE" "$ROPE_RATIO" "$BRANCH_STR" "$VAL_BRANCH" "$LOSS" "$PPL"
                log "[EVAL] Recorded: heads=$NUM_HEADS, rope=$ROPE_RATIO, train=$BRANCH_STR, eval=$VAL_BRANCH, loss=$LOSS"
            fi
...
log ""
log "============================================================================"
log "ParallelMind 消融实验 - Attention Head 数量测试"
log "Started/Resumed at $(date)"
log "Log directory: $LOG_DIR"
log "Force rerun: $FORCE_RERUN"
log ""
log "实验配置:"
log "  num_heads: ${HEAD_CONFIGS[*]}"
log "  head_dim: $HEAD_DIM (固定)"
...
log "  总实验数: ${#HEAD_CONFIGS[@]} heads × ${#ROPE_RATIOS[@]} ratios × ${#TRAIN_CONFIGS[@]} configs = $TOTAL_EXPERIMENTS 个"
log "============================================================================"
...
# 遍历所有 head 配置
for NUM_HEADS in "${HEAD_CONFIGS[@]}"; do
    HIDDEN_SIZE=$((NUM_HEADS * HEAD_DIM))

    log ""
    log "############################################################################"
    log "# NUM_HEADS = $NUM_HEADS, HIDDEN_SIZE = $HIDDEN_SIZE"
    log "############################################################################"
    ...
    for i in "${!ROPE_RATIOS[@]}"; do
        ...
        record_loss ""
        record_loss "========================================"
        record_loss "NUM_HEADS=$NUM_HEADS, HIDDEN_SIZE=$HIDDEN_SIZE, ROPE_2D_RATIO=$ROPE_RATIO"
        record_loss "========================================"
        ...
        for CONFIG in "${TRAIN_CONFIGS[@]}"; do
            ...
            log ">>> HEADS=$NUM_HEADS, ROPE=$ROPE_RATIO, TRAIN_BRANCH=$BRANCH_STR"

            record_loss ""
            record_loss "--- EXPERIMENT $EXPERIMENT_COUNT / $TOTAL_EXPERIMENTS ---"
            record_loss "HEADS=$NUM_HEADS, ROPE=$ROPE_RATIO, TRAIN_BRANCH=$BRANCH_STR"
            ...
            for VAL_BRANCH in "${EVAL_BRANCHES[@]}"; do
                ...
                run_evaluation "$MODEL_PATH" "$NUM_HEADS" "$HIDDEN_SIZE" "$ROPE_RATIO" "$ROPE_STR" "$BRANCH_STR" "$VAL_BRANCH"
            done
...
echo "To force rerun all, use: ./run_ablation_heads.sh --force"
echo "=========================================="
