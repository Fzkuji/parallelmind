...
from src.model.model_lora import apply_lora, save_lora, load_lora
...
from src.data.grouped_parallel_dataset import GroupedParallelDataset
from src.data.grouped_parallel_collator import GroupedParallelCollator
...
def get_lr(current_step, total_steps, lr):
    return lr / 10 + 0.5 * lr * (1 + math.cos(math.pi * current_step / max(1, total_steps)))
    ...
    for step, batch in enumerate(train_loader):
        ...
        if step % args.log_interval == 0:
            ...
            if wandb is not None and (not ddp or dist.get_rank() == 0):
                wandb.log({"loss": loss.item() * args.accumulation_steps, "lr": optimizer.param_groups[-1]["lr"]})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LoRA finetune on HuggingFace model with grouped parallel data")
    parser.add_argument("--base_model", type=str, required=True)
    parser.add_argument("--tokenizer_path", type=str, default=None)
    parser.add_argument("--out_dir", type=str, default=os.path.join(root_path, "out"))
    parser.add_argument("--data_path", type=str, required=True)
    parser.add_argument("--max_parallel_samples", type=int, default=None)
    parser.add_argument("--lora_name", type=str, default="hf_grouped_lora")
    parser.add_argument("--lora_rank", type=int, default=8)
    parser.add_argument("--epochs", type=int, default=3)
    parser.add_argument("--batch_size", type=int, default=4, help="grouped sample 数量")
    parser.add_argument("--learning_rate", type=float, default=1e-4)
    parser.add_argument("--accumulation_steps", type=int, default=1)
    parser.add_argument("--grad_clip", type=float, default=1.0)
    parser.add_argument("--device", type=str, default="cuda:0" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--use_wandb", action="store_true")
    parser.add_argument("--wandb_project", type=str, default="ParallelMind-HF-Grouped-LoRA")
    ...
    parser.add_argument("--load_lora", type=str, default=None)
    parser.add_argument("--model_tag", type=str, default="grouped")
    ...
    parser.add_argument("--align_to", type=str, choices=["left", "right"], default="left")
    ...
    Logger(f"加载 grouped parallel 数据集: {args.data_path}")
    train_ds = GroupedParallelDataset(data_path=args.data_path, max_samples=args.max_parallel_samples)
    train_sampler = DistributedSampler(train_ds, shuffle=True) if ddp else None

    collator = GroupedParallelCollator(
        tokenizer,
    ...
    if args.use_wandb and (not ddp or dist.get_rank() == 0):
        ...
        wandb.init(project=args.wandb_project, name=f"HF-Grouped-LoRA-{args.lora_name}")

    Logger("=" * 80)
    Logger("开始 grouped parallel 训练...")
    Logger("=" * 80)
